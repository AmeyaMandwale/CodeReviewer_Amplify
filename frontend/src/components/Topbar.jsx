...
// import { Search, Bell, User, Moon, Sun, LogOut, Settings, Menu, LogIn } from 'lucide-react';
...
const Topbar = ({ onMenuClick }) => {
  ...
  const [showUserMenu, setShowUserMenu] = useState(false);
  const [isDark, setIsDark] = useState(false);
  const [isLoggedIn, setIsLoggedIn] = useState(true); // Default to true to show all existing features
  const navigate = useNavigate();
    ...
    <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 px-4 lg:px-6 py-4">
      <div className="flex items-center justify-between">
        ...
        >
         ...
         {/* CTPL Code Reviewer Header - Add this section */}
  <div className="hidden md:block">
    <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">CTPL Code Reviewer</h1>
  </div>
        ...
        <div className="flex items-center space-x-2 lg:space-x-4">
          ...
          {/* Notifications - Show always (as in original) but conditionally render based on login */}
          <div className="relative">
            <button
              onClick={() => setShowNotifications(!showNotifications)}
              className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors relative"
          ...
          {/* User Menu and Sign In Button - Show both but conditionally */}
          <div className="flex items-center space-x-2">
            ...
            {/* User Menu - Show always (as in original) but you can conditionally render based on login */}
            <div className="relative">
              <button
                onClick={() => setShowUserMenu(!showUserMenu)}
                className="flex items-center space-x-2 p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
